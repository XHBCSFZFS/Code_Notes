...
在 `pgsql` 可以直接使用 `CREATE OR REPLACE VIEW` 创建新视图或替换现有视图

```sql
-- 删除视图
DROP VIEW view_name; -- pgsql与sqlserver用法一样
```
...
例如 : `nginx`  `OpenResty` `apache` 如果是安装在docker内请先停止运行 , 因为 Certbot 需要绑定到 80 端口来完成验证 .
...
```shell
sudo certbot certonly --standalone -d 在这里加上您的域名或者子域名
```

#### 4. **配置 Web 服务器**

根据`certbot`反馈的`Let's Encrypt`证书位置去配置web服务器

一般位于 `  /etc/letsencrypt/live/申请的域名/  `路径内
...
```vb
Dim planTypeMapping As New Dictionary(Of String, String)()
planTypeMapping.Add("主计划", "A")
planTypeMapping.Add("分卡_首序", "B")
planTypeMapping.Add("分卡_中间工序", "C")
planTypeMapping.Add("分卡_选中序", "D")
planTypeMapping.Add("补卡_首序到底", "E")
If planTypeMapping.ContainsKey(key) Then  '是否存在?
    csDataRow("F_") = planTypeMapping(key)
End If
```
...
读取Excel文件并导出数据为Excel文件 使用 `Spire.Xls `
...
```vb
    ...
    ''' </summary>
    ''' <returns></returns>
    Private Function PromptUserToSelectExcelFile() As String
        Using csOpenFileDialog As New OpenFileDialog
            With csOpenFileDialog
...
```vb
    ...
    ''' <returns></returns>
    Private Function IsValidExcelFilePath(filePath As String) As Boolean
        If String.IsNullOrEmpty(filePath) Then
            MsgBox("未选择文件，请重新选择！", MsgBoxStyle.OkOnly, "提示信息")
            Return False
        End If

        If Not System.IO.File.Exists(filePath) Then
            MsgBox("选择的文件不存在，请重新选择！", MsgBoxStyle.OkOnly, "提示信息")
            Return False
        End If

        FileName = System.IO.Path.GetFileName(filePath).ToString().Trim().Replace(" ", "").Replace(".xlsx", "").Replace(".xls", "")
        Dim fileExtension = System.IO.Path.GetExtension(filePath).ToLower()
        If fileExtension <> ".xls" AndAlso fileExtension <> ".xlsx" Then
            MsgBox("选择的文件不是有效的Excel文件，请重新选择！", MsgBoxStyle.OkOnly, "提示信息")
            Return False
        End If
...
```vb
Dim workbook As New Workbook() '创建对象Workbook
workbook.LoadFromFile(selectedFilePath) '通过路径加载工作表
```
...
```vb
    ...
    ''' <returns></returns>
    Private Function ValidateWorkbook(ByVal workbook As Workbook) As Boolean
        ' 检查工作表的数量
        If workbook.Worksheets.Count > 1 Then
            MsgBox("选定的文件包含多个工作表，请重新选择！", MsgBoxStyle.OkOnly, "提示信息")
            Return False
        End If
        ...
        ' 检查新的第一行第一列的值
        Dim firstCellValue As String = worksheet.Range(1, 1).Value?.Trim()
        If firstCellValue <> MODEL_COLUMN_NAME Then
            MsgBox("第一行的第一个值不是'型号'，请检查！", MsgBoxStyle.OkOnly, "提示信息")
            Return False
        End If
...
```vb
Dim worksheet As Worksheet = workbook.Worksheets(0) '拿到第一个工作表
```
...
```vb
ProcessWorksheet(worksheet) '处理工作表格式
```

> 我这里将不符合数据规范的行列都删掉 并且 对Excel单元格内的公式进行计算 将不规范的值替换成0  给指定列添加标识符 方便后续转成`DataTable`处理
...
```vb
    ...
    Private Sub ProcessWorksheet(ByRef worksheet As Worksheet)
        ' 处理行 删除值为“小计”、“合计”或为空的所有行
        For i As Integer = worksheet.LastRow To 1 Step -1
            Dim cellValue As String = worksheet.Range(i, 1).Value?.Trim()
            If String.IsNullOrEmpty(cellValue) OrElse cellValue = "小计" OrElse cellValue = "合计" Then
                worksheet.DeleteRow(i)
            End If
        Next

        ' 处理列 删除列名不在列表中或为空的所有列
        For i As Integer = worksheet.Columns.Length To 1 Step -1
            Dim colName As String = worksheet.Range(1, i).Value?.Trim()
            If String.IsNullOrEmpty(colName) OrElse Not validColumnNames.Contains(colName) Then
                worksheet.DeleteColumn(i)
            End If
        Next

        ' 拿到处理完的表格行列信息
        Dim workColumn As Integer = worksheet.Columns.Length

        ' 为列名添加标识符
        For i As Integer = 2 To workColumn ' 从2开始，因为第一列是“型号”
            Dim colName As String = worksheet.Range(1, i).Value
            If i <= 17 Then ' 2-17列（不含“型号”列）
                worksheet.Range(1, i).Value = "上日库存_" & colName
            ElseIf i <= 33 Then ' 18-33列
                worksheet.Range(1, i).Value = "本日进仓_" & colName
            ElseIf i <= 49 Then ' 34-49列
                worksheet.Range(1, i).Value = "本日发出_" & colName
            ElseIf i <= 65 Then ' 50-65列
                worksheet.Range(1, i).Value = "实际库存_" & colName
            End If
        Next

        ' 将公式替换成计算的值 并且将 "-" 或 " " (空格)转换成0
        For col As Integer = 1 To workColumn
            For row As Integer = 1 To worksheet.LastRow
                Dim cell As CellRange = worksheet.Range(row, col)
                Dim cellValue As Object

                ' 如果单元格包含公式，获取计算后的值
                If cell.HasFormula Then
                    cellValue = cell.FormulaValue
                Else
                    cellValue = cell.Value
                End If

                ' 检查单元格的值是否是 "-" 或 " " (空格)，并相应地处理
                If cellValue.ToString().Trim() = "-" OrElse String.IsNullOrWhiteSpace(cellValue.ToString()) Then
                    worksheet.Range(row, col).Value = 0
                Else
                    worksheet.Range(row, col).Value = cellValue
                End If
            Next
        Next
    End Sub
...
### 7. 读取数据然后写入`DataTable`

我这里将`Worksheet`转换成`DataTable`处理 去掉列内空格 再创建一个`DataTable`来写入`Worksheet`的内容 根据需求 最后返回处理完成新的`DataTable`
...
```vb
    ...
    ''' <param name="worksheet"></param>
    ''' <returns></returns>
    Private Function ConvertWorksheetToDataTable(ByVal worksheet As Worksheet) As DataTable
        ' 转换为DataTable
        Dim WorkDataTable As DataTable = worksheet.ExportDataTable()

        '处理所有列名称携带 空格 的情况
        For Each column As DataColumn In WorkDataTable.Columns
            column.ColumnName = column.ColumnName.Trim()
        Next

        '创建一个新的DataTable 用作导出
        Dim newDataTable As New DataTable
        '设置表头
        newDataTable.Columns.Add(MODEL_COLUMN_NAME)
        newDataTable.Columns.Add("厂家")
        newDataTable.Columns.Add("上日库存")
        newDataTable.Columns.Add("本日进仓")
        newDataTable.Columns.Add("本日发出")
        newDataTable.Columns.Add("实际库存")

        ' 对每一行数据进行处理
        For Each row As DataRow In WorkDataTable.Rows
            Dim ModelDetails As String = row(MODEL_COLUMN_NAME).ToString() ' 当前行的型号明细

            ' 对于每一个厂家，我们创建一个新的行并添加到 newDataTable
            For Each ManufacturerDetails As String In fixedManufacturers
                Dim newRow As DataRow = newDataTable.NewRow()
                newRow(MODEL_COLUMN_NAME) = ModelDetails
                newRow("厂家") = ManufacturerDetails
                newRow("上日库存") = If(WorkDataTable.Columns.Contains("上日库存_" & ManufacturerDetails), row("上日库存_" & ManufacturerDetails).ToString().Trim(), 0)
                newRow("本日进仓") = If(WorkDataTable.Columns.Contains("本日进仓_" & ManufacturerDetails), row("本日进仓_" & ManufacturerDetails).ToString().Trim(), 0)
                newRow("本日发出") = If(WorkDataTable.Columns.Contains("本日发出_" & ManufacturerDetails), row("本日发出_" & ManufacturerDetails).ToString().Trim(), 0)
                newRow("实际库存") = If(WorkDataTable.Columns.Contains("实际库存_" & ManufacturerDetails), row("实际库存_" & ManufacturerDetails).ToString().Trim(), 0)
...
### 8. DataTable转成Excel导出

其中有对数据进行基础校验 使用`SaveFileDialog`对数据进行保存

```shell
  	''' <summary>
    ''' 处理dataTable导出defaultFileName转换后Excel文件
    ''' </summary>
    ''' <param name="dataTable"></param>
    Private Sub SaveDataTableAsExcel(ByVal dataTable As DataTable, ByVal fileName As String)
        ' 保存处理后的数据表到新的 Excel 文件
        ...
        Dim newWorksheet As Worksheet = newWorkbook.Worksheets(0)
        newWorksheet.InsertDataTable(dataTable, True, 1, 1)

        Using saveFileDialog As New SaveFileDialog()
            saveFileDialog.Filter = "Excel 工作簿 (*.xls;*.xlsx)|*.xls;*.xlsx"
            saveFileDialog.Title = "保存为"
            saveFileDialog.FileName = fileName & "转换后.xlsx"
            If saveFileDialog.ShowDialog() = DialogResult.OK Then
                newWorkbook.SaveToFile(saveFileDialog.FileName)
                MsgBox("文件保存成功！", MsgBoxStyle.OkOnly, "提示信息")
                cleanClose()
            End If
        End Using
    End Sub
...
Public Class Frm_ExcelEdit
    '文件名
    Private FileName As String = String.Empty
    '必要信息
    Private Const MODEL_COLUMN_NAME As String = "型号"
    ' 固定的列名字段信息
    Private validColumnNames As New List(Of String) From {MODEL_COLUMN_NAME, "标准", "杭叉", "龙工", "柳工", "宝鸡", "江淮", "瑞创", "中联", "大连", "厦门", "宝骊", "海斯特", "斗山", "梯易优", "中力", "台励福", "吉鑫祥"}
    ' 固定的厂家列表
    Private fixedManufacturers As New List(Of String) From {"标准", "杭叉", "龙工", "柳工", "宝鸡", "江淮", "瑞创", "大连", "厦门", "宝骊", "海斯特", "斗山", "梯易优", "中力", "台励福", "吉鑫祥", "中联"}

    '点击选择文件按钮
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Try
            Dim selectedFilePath = PromptUserToSelectExcelFile() '让用户选择Excel文件返回路径

            If Not IsValidExcelFilePath(selectedFilePath) Then Return

            TextBox1.Text = FileName '文件名显示给用户

            Dim workbook As New Workbook() '创建对象Workbook
            workbook.LoadFromFile(selectedFilePath) '通过路径加载工作表

            If Not ValidateWorkbook(workbook) Then Return ' 检验工作表

            Dim worksheet As Worksheet = workbook.Worksheets(0) '拿到第一个工作表
            ProcessWorksheet(worksheet) '处理工作表格式

            Dim data As DataTable = ConvertWorksheetToDataTable(worksheet) '处理工作表并转换成DataTable

            SaveDataTableAsExcel(data, FileName) '保存导出成Excel文件
    ...
    Private Sub HandleError(ex As Exception)
        MsgBox(ex.Message, MsgBoxStyle.OkOnly, "提示信息")
        cleanClose()
    End Sub
    ...
    ''' </summary>
    Private Sub ResetUI()
        'TextBox1.Text = String.Empty '清空显示的文件名
